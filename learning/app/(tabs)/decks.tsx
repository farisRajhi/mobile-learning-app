...
const DecksScreen: React.FC = () => {
  ...
  const createSubject = async (): Promise<void> => {
    ...
    try {
      const response = await api.post<Subject>('/subjects', { name: newName });
        ...
        {subjects.map((subject) => (
          <View key={subject.id} style={styles.card}>
            ...
            <TouchableOpacity style={styles.addTopicBtn}>
              ...
              <Text style={[styles.addTopicText, { color: GREEN }]}>Add Topic</Text>
            </TouchableOpacity>
          </View>
        ))}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  addTopicText: { fontSize: 14, marginLeft: 6 },
  tabBar: {
    position: 'absolute',
    bottom: 0,
    width,
  ...
  cancelBtn: { backgroundColor: '#fff', borderWidth: 1, borderColor: GREEN },
